...
# Task: fx/sprites.js — real character art

You own **`fleet_view/fx/sprites.js`** and nothing else.

## What it does
world.html draws characters as canvas rectangles — deliberately, so the data could be
proven first. Replace them with real pixel-art sprites: a distinct look per agent kind
(claude, codex, grok, and a generic fallback), drawn at the same positions.

## How to replace rather than duplicate
You cannot edit world.html, so you cannot switch its drawing off. Draw over it in
`drawOver`: cover each character with your sprite. Cover it properly — a half-covered
rectangle peeking out is worse than the rectangle alone. If your sprite cannot cover a
given character cleanly, skip that one and leave the primitive.

Say plainly in your PR if this approach fights the contract; a follow-up can add a
`suppressDefaultCharacters` flag. **Do not add that flag yourself** — world.html belongs
to another crewmate this round.

## Must stay legible at a glance
The states already mean something and your art must preserve every one:
- `working` — at the desk, active
- `idle` — upright, unoccupied
- `stalled` — asleep
- `presence: "ghost"` — translucent, drifting, no shadow
- holding an artifact — visibly holding a document

If art makes any of these harder to tell apart, the art is wrong.

## Constraints
- Procedural canvas drawing only. No image files, no base64 blobs, no fonts, no libraries.
- It must stay readable small — rooms shrink when the fleet is large.
- Budget the cost: this redraws every frame for every character.

## Verify
Screenshot with several kinds and states visible at once, including a ghost.
